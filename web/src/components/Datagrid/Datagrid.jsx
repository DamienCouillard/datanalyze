...
export default class CustomModal extends Component {
  ...
  refreshRecords = (from) => {
      ...
      axios
      .get(`http://localhost:9200/${this.state.activeItem}/_search?size=15&from=${from}`)
      .then(res => { this.getRecords(res.data); this.getMapping();})
  ...
  refreshSize = () => {
    ...
    .then(res => { var len = res.data['_all']['primaries']['docs']['count']; this.setState({size : len})})
    .catch(err => console.log(err));
};
    ...
    mapHeader = () => {
        return this.state.mapping.map(item => {return <th>{item}</th>})
    }
    ...
    mapRecord = () => {
        ...
        for (var i in records) {
            ...
            toMap.push(
            temp.map(item => {return <td>{item}</td>})
            )
        }
        return toMap.map(item => {return <tr>{item}</tr>})
   ...
   pageNext(e){
    ...
    pagination = (i) => {
        ...
        if (i < maxPage-2) {
            items.push(<Pagination.Item key={i} active={i === active}>{i}</Pagination.Item>);
            items.push(<Pagination.Item key={i+1} active={i+1 === active}>{i+1}</Pagination.Item>);
        } else {
            items.push(<Pagination.Item key={maxPage-3} active={maxPage-3 === active}>{maxPage-3}</Pagination.Item>);
            items.push(<Pagination.Item key={maxPage-2} active={maxPage-2 === active}>{maxPage-2}</Pagination.Item>);
        }
        if (i < maxPage-3) {
            items.push(<Pagination.Ellipsis/>)
        }
        items.push(<Pagination.Item key={maxPage-1} active={maxPage-1 === active}>{maxPage-1}</Pagination.Item>);
        items.push(<Pagination.Item key={maxPage} active={maxPage === active}>{maxPage}</Pagination.Item>);
        return (
            <Pagination>
